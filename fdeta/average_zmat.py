...
def get_all_internal_coords(aligned_fn: str, int_coords_file: str = "internal_coordinates.npz", save: bool =True, dec_digits: int = 3):
    ...
    int_coords_file : str
        Filename for writing all the internal coordinates. ".npy" and ".npz" can be detected, otherwise ".txt" separate files are used.
    save : bool
    ...
    tuple(array, array, array)
        Bonds array(n_atoms,n_frames), Angles array(n_atoms,n_frames), Dihedrals array(n_atoms,n_frames)
    """
    ...
    if save:
        ...
        if int_coords_file[:-4]==".npy":
            ...
            print("saved bonds,angles,dihedrals in {} respectively".format(", ".join([int_coords_file[:-4]+i+int_coords_file[-4:] for i in["_bonds","_angles","_dihedrals"]])))
    return (bonds, angles, dih)

def correct_quasilinears(dih: np.darray = np.array([]), dih_c: np.darray = np.array([]), dih_std: np.darray = np.array([]), dih_c_std: np.darray = np.array([])):
...
def average_int_coords(aligned_fn: str ="", int_coord_file: str = "", out_file: str = "averaged.xyz", overwrite: bool = False, view: bool = True, correct_quasilinears: bool = True, detect_rotate: str = "std", fix_rot: str = "pick_first"):
    ...
    if int_coord_file == "":
        ...
        if len(ic_files) == 0:
            ...
            if lenlist == [1,1,1]:
                ...
                try:
                    bonds, angles, dih = get_all_internal_coords(aligned_fn = "aligned0.xyz")  #use correct default name for aligned xyz
                except:
    ...
    if int_coord_file[-4:] == ".txt":
        try:
            ...
            try:
                ...
                raise FileNotFoundError("Cannot read it neither as cartesian trajectory nor as saved bonds,angles,dihedrals")
    if len(txtfiles) == 3:
