...
import Kabsch as kb
import numpy as np
from Class_Trajectory import Trajectory

class Trajectory_Analysis(Trajectory):
    """
    """
...
#        print self.rmsd(self.structure,self.reference_structure)
...
#        print self.rmsd(self.structure_aligned,self.reference_structure)
...
#        kb.write_coordinates(self.structure_atoms, self.structure_aligned)
...
#       Collecting all coordinates through all frames for a given element ielement
            ...
            #print "how many coordinates? ", len(coordinates)
            #print "coordinates? ", coordinates
            self.pcf[ielement]=np.histogramdd(coordinates,bins=50)
#            This variable stores the coordinates to be used by other classes and modules
#            self.Coordinates_for_pcf=coordinates.copy()

#       Assuming that the order of molecular_id is not changed with the frame number in trajectory file


#        Center of the box considered to be the center of the center of the mass of the system
#                    Converting elements' names to the atomic weights
#        self.mass=np.vectorize(self.nametomass)(self.Elements)
#        self.center_of_mass=[self.compute_center_of_mass(self.mass,self.Frames_aligned[iframe][:,:3]) for iframe in range(self.Total_number_of_frames)]
...
#        Getting indicies for solute (molecules to be excluded)
        self.Solute_index=np.where(self.Frames[0,:,3]==molecular_id)[0]
        #print "Solute index =", self.Solute_index
#        Getting indicies for each type of element
        for ielement in set(self.Elements):
...
#            Excluding atoms of solute from the list of atoms
            ...
            #print "mask ", mask
            self.Index_of_elements[ielement]=self.Index_of_elements[ielement][mask]
            #print "Index of elements = ", self.Index_of_elements[ielement]
#       Collecting all coordinates through all frames for a given element ielement
            coordinates=self.Frames_aligned[:,self.Index_of_elements[ielement]].reshape(self.Total_number_of_frames*self.Index_of_elements[ielement].size,3)
            #print "how many coordinates? ", len(coordinates)
            #print "bins: ", bins
            ...
            #print "pcf_range", pcf_range
            #self.pcfADF[ielement]=np.histogramdd(coordinates,range=pcf_range,bins=np.array([5,5,5]))
            self.pcfADF[ielement]=np.histogramdd(coordinates,range=pcf_range,bins=bins)
            #print "sum of all pcfADF = ", sum(self.pcfADF[ielement][0])
            #print "size ", self.pcfADF[ielement][0].size
            #print "shape ", self.pcfADF[ielement][0].shape
            #print "pcf inside:"
            #print self.pcfADF[ielement]
        return self.pcfADF
